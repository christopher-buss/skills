...
const me = i((e, t) => Re($e(t, e, "utf8")), "readJsonc"), C = Symbol("implicitBaseUrl"), I = "${configDir}", Se = i(() => {
	...
	return u = m.join(e, "..", u), n?.set(o, u), u;
}, "resolveFromPackageJsonPath"), ne = "package.json", te = "tsconfig.json", Je = i((e, t, s) => {
	let n = e;
	if (e === ".." && (n = m.join(n, te)), e[0] === "." && (n = m.resolve(t, n)), m.isAbsolute(n)) {
		if (E(s, n)) {
	...
	const g = fe(m.resolve(t), m.join("node_modules", u), s);
	...
	const d = m.join(g, a), _ = d.endsWith(".json");
	if (!_) {
		const p = `${d}.json`;
		if (E(s, p)) return p;
	}
...
}, "resolveExtendsPath"), se = i((e, t) => K(m.relative(e, t)), "pathRelative"), we = [
	"files",
	"include",
	"exclude"
], de = i((e, t, s) => {
	const n = m.join(t, s);
	return h(m.relative(e, n)) || "./";
	...
	s.add(o);
	const l = m.dirname(o), u = ve(o, n, s);
	delete u.references;
	...
	if (n.compilerOptions) {
		...
		for (const a of ["baseUrl", "rootDir"]) {
			const r = l[a];
			if (r && !r.startsWith(I)) l[a] = se(o, m.resolve(o, r));
		}
		for (const a of be) {
			let r = l[a];
	...
	if (n.include ? (n.include = n.include.map(h), n.files && delete n.files) : n.files && (n.files = n.files.map((l) => l.startsWith(I) ? l : K(l))), n.watchOptions) {
		...
		l.excludeDirectories && (l.excludeDirectories = l.excludeDirectories.map((u) => h(m.resolve(o, u)))), l.excludeFiles && (l.excludeFiles = l.excludeFiles.map((u) => h(m.resolve(o, u)))), l.watchFile && (l.watchFile = l.watchFile.toLowerCase()), l.watchDirectory && (l.watchDirectory = l.watchDirectory.toLowerCase()), l.fallbackPolling && (l.fallbackPolling = l.fallbackPolling.toLowerCase());
	}
	return n;
}, "_parseTsconfig"), H = i((e, t) => {
	if (e.startsWith(I)) return h(m.join(t, e.slice(12)));
	...
	const s = m.resolve(e), n = ve(s, t), o = m.dirname(s), { compilerOptions: l } = n;
	if (l) {
		for (const a of Me) {
			const r = l[a];
			if (r) {
	...
	substitutions: o.map((l) => {
		if (Te(l, `Substitution '${l}' in pattern '${n}' can have at most one '*' character.`), !t && !q.test(l) && !m.isAbsolute(l)) throw new Error("Non-relative paths are not allowed when 'baseUrl' is not set. Did you forget a leading './'?");
		return m.resolve(s, l);
	})
})), "parsePaths"), Ze = i((e) => {
	...
	if (!s && !n) return null;
	const o = C in t && t[C], l = m.resolve(m.dirname(e.path), s || o || "."), u = n ? Ye(n, s, l) : [];
	return (a) => {
		if (q.test(a)) return [];
		const r = [];
		for (const _ of u) {
...
const Ae = X((e) => {
...
}, "isFsCaseSensitive"), { join: ye } = m.posix, oe = {
	ts: [
		".ts",
		".tsx",
		".d.ts"
	...
	if (!m.isAbsolute(t)) throw new Error("The tsconfig path must be absolute");
	De && (t = h(t));
	const n = m.dirname(t), { files: o, include: l, exclude: u, compilerOptions: a } = e, r = i((T) => m.isAbsolute(T) ? T : ye(n, T), "resolvePattern"), g = o == null ? void 0 : o.map(r), v = Ce(a), d = s ? "" : "i", p = (u || en(a)).map((T) => {
	...
	return (T) => {
		if (!m.isAbsolute(T)) throw new Error("filePath must be absolute");
...
function resolveViaReferences(directory, configPath, targetFile) {
	const { references } = pe(configPath);
...
function typecheckStopDecision(input) {
	...
	if (input.stopAttempts >= DEFAULT_MAX_STOP_ATTEMPTS) return { reason: `Could not fix type errors in: ${input.errorFiles.join(", ")}` };
	return {
		decision: "block",
...
//#endregion
export { runTypeCheck as a, writeTypecheckStopAttempts as c, resolveTsconfig as i, isTypeCheckable as n, typeCheck as o, readTypecheckStopAttempts as r, typecheckStopDecision as s, clearTypecheckStopAttempts as t };